        ...
        <div class="rounded-xl border border-neutral-700 bg-neutral-800/50 p-6">
            ...
            <div class="flex flex-wrap gap-3">
                @foreach ($balances as $balance)
                    <div wire:key="balance-{{ $balance['user']->id }}" class="flex items-center gap-2 p-2 rounded-lg border border-neutral-700">
            ...
            @if (count($transfers) > 0)
                <div class="space-y-2">
                    @foreach ($transfers as $transfer)
                        <div wire:key="transfer-{{ $transfer['from']->id }}-{{ $transfer['to']->id }}" class="flex items-center gap-3 p-3 rounded-lg border border-neutral-700">
